...
class ProductDetailSlugView(ObjectViewedMixin, DetailView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        cart_obj, new_obj = Cart.objects.new_or_get(self.request)
        context['cart'] = cart_obj
        categories = self.get_object().category
        print(categories)
        same_price = self.get_object().price

        if categories:
            qs = Product.objects.filter(category__name__icontains=categories).exclude(pk=(self.get_object()).pk)
            context['same_categories'] = qs
            
        if same_price:
            qs = Product.objects.filter(price=same_price).exclude(pk=(self.get_object()).pk)
            context['same_price'] = qs
